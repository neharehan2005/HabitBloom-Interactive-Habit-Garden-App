import React, { useMemo, useState, useEffect, useRef } from 'react';
import {
  View,
  Text,
  StyleSheet,
  ScrollView,
  TouchableOpacity,
  StatusBar,
  ActivityIndicator,
  Animated,
  Modal,
  Pressable,
  Switch,
} from 'react-native';
import { useSelector } from 'react-redux';
  import { useContext } from 'react';
import { ThemeContext } from '../../components/ThemeContext';

// Example COLORS object
const COLORS = {
  background: '#FAFAFA',
  primary: '#F4511E',
  card: '#FFFFFF',
  textDark: '#212121',
  textLight: '#757575',
  lightOrange: '#e2f1a8',
};

// Achievements
const achievements = [
  { id: 'starter', icon: '📦', name: 'Starter', desc: 'Created your first habit' },
  { id: 'streak3', icon: '🔥', name: '3 Day Streak', desc: 'Maintained a 3 day streak' },
  { id: 'consistent', icon: '🌱', name: 'Consistent', desc: '7 days of consistency' },
  { id: 'master', icon: '⭐', name: 'Master', desc: 'Completed a 30 day challenge' },
];

// Habit type example
type Habit = {
  id: string;
  name: string;
  streak: number;
  duration: number;
};

// ─── NEW: Weekly progress data (days of week) ───────────────────────────────
const WEEK_DAYS = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];

// ─── NEW: Notification settings ─────────────────────────────────────────────
type NotifSettings = {
  dailyReminder: boolean;
  streakAlert: boolean;
};

// ─── NEW: Milestone levels ───────────────────────────────────────────────────
const MILESTONES = [
  { label: 'Seedling', minStreak: 0, icon: '🌱', color: '#81C784' },
  { label: 'Sprout', minStreak: 3, icon: '🪴', color: '#4CAF50' },
  { label: 'Bloomer', minStreak: 7, icon: '🌸', color: '#E91E63' },
  { label: 'Champion', minStreak: 14, icon: '🏅', color: '#FF9800' },
  { label: 'Legend', minStreak: 30, icon: '🔥', color: '#F44336' },
];

function getMilestone(bestStreak: number) {
  let current = MILESTONES[0];
  for (const m of MILESTONES) {
    if (bestStreak >= m.minStreak) current = m;
  }
  return current;
}

// ─── NEW: Animated progress bar ─────────────────────────────────────────────
function AnimatedBar({ percent, color }: { percent: number; color: string }) {
  const anim = useRef(new Animated.Value(0)).current;
  useEffect(() => {
    Animated.timing(anim, {
      toValue: percent,
      duration: 900,
      useNativeDriver: false,
    }).start();
  }, [percent]);
  const width = anim.interpolate({ inputRange: [0, 100], outputRange: ['0%', '100%'] });
  return (
    <View style={barStyles.track}>
      <Animated.View style={[barStyles.fill, { width, backgroundColor: color }]} />
    </View>
  );
}

const barStyles = StyleSheet.create({
  track: { height: 8, backgroundColor: '#E0E0E0', borderRadius: 4, overflow: 'hidden', flex: 1 },
  fill: { height: '100%', borderRadius: 4 },
});

export default function ProfileScreen() {
  const [quote, setQuote] = useState<{ content: string; author: string } | null>(null);
  const [loading, setLoading] = useState(true);

  // Get habits from Redux
  const habits: Habit[] = useSelector((state: any) => state.habits.habits);

  // Compute stats
  const stats = useMemo(() => {
    const totalHabits = habits.length;
    const completedHabits = habits.filter(h => h.streak >= h.duration).length;
    const bestStreak = habits.length > 0 ? Math.max(...habits.map(h => h.streak)) : 0;
    const totalStreaks = habits.reduce((sum, h) => sum + (h.streak || 0), 0);
    return { totalHabits, completedHabits, bestStreak, totalStreaks };
  }, [habits]);

  // // Theme state
  // const [selectedTheme, setSelectedTheme] = useState(themes[2]);


  const themeContext = useContext(ThemeContext);
  if (!themeContext) return null;

  const { selectedTheme, setSelectedTheme, themes } = themeContext;

  useEffect(() => {
    setLoading(true);
    fetch('https://motivational-spark-api.vercel.app/api/quotes/random')
      .then(response => response.json())
      .then(data => {
        setQuote({ content: data.quote, author: data.author || '' });
        setLoading(false);
      })
      .catch(error => {
        console.error('Error fetching the quote:', error);
        setQuote({ content: '', author: '' });
        setLoading(false);
      });
  }, []);
  useEffect(() => {
  // Check notification settings when app opens
  if (notifSettings.dailyReminder) {
    alert('⏰ Daily Reminder: Don’t forget to update your habits today!');
  }

  if (notifSettings.streakAlert) {
    const bestStreak = stats.bestStreak;
    if (bestStreak > 0) {
      alert(`🔥 Streak Alert: Your best streak is ${bestStreak} days! Keep it up!`);
    }
  }
}, []); // empty dependency = run only once when screen mounts

  // ─── NEW: State for added features ─────────────────────────────────────────
  const [notifSettings, setNotifSettings] = useState<NotifSettings>({
    dailyReminder: true,
    streakAlert: true,
    
  });
  const [showNotifModal, setShowNotifModal] = useState(false);

  // NEW: Simulated weekly activity (7 booleans — in real app, compute from habit logs)
  const weeklyActivity: boolean[] = useMemo(() => {
    // Mark today and a few past days as active based on bestStreak
    const streak = Math.min(stats.bestStreak, 7);
    return WEEK_DAYS.map((_, i) => i >= 7 - streak);
  }, [stats.bestStreak]);

  // NEW: Milestone
  const milestone = getMilestone(stats.bestStreak);

  // NEW: Completion percentage for progress ring label
  const completionPct =
    stats.totalHabits > 0 ? Math.round((stats.completedHabits / stats.totalHabits) * 100) : 0;

  // NEW: Habit list sorted by streak (top habits leaderboard)
  const topHabits = useMemo(
    () => [...habits].sort((a, b) => b.streak - a.streak).slice(0, 3),
    [habits],
  );

  // ─── RENDER ──────────────────────────────────────────────────────────────────
  return (
    <View style={[styles.root, { backgroundColor: selectedTheme.bg }]}>
      <StatusBar barStyle="dark-content" />
      <ScrollView contentContainerStyle={styles.container}>

        {/* ── Profile Header (UNCHANGED) ─────────────────────────────────── */}
        <View style={[styles.header, { backgroundColor: selectedTheme.primary }]}>
          <Text style={{ fontSize: 36 }}>👨‍💼 / 👩‍💼</Text>
          <Text style={styles.name}>HabitBloom User</Text>
          <Text style={styles.tagline}>Growing one day at a time 🌱</Text>

        </View>
        {/* <View style={styles.levelBadge}>
           <Text style={styles.levelText}>Level {Math.floor(stats.totalStreaks / 10) + 1}</Text>
         </View> */}

        {/* ── Motivation Container (UNCHANGED) ──────────────────────────── */}
        <View
          style={[
            styles.motivationContainer,
            { backgroundColor: selectedTheme.motivationBg, borderColor: selectedTheme.primary },
          ]}
        >
          {loading ? (
            <ActivityIndicator size="small" color={selectedTheme.primary} />
          ) : (
            <>
              <Text style={[styles.quoteText, { color: selectedTheme.textContrast }]}>
                "{quote?.content}"
              </Text>
              {quote?.author ? (
                <Text style={[styles.quoteAuthor, { color: selectedTheme.textContrast }]}>
                  - {quote.author}
                </Text>
              ) : null}
            </>
          )}
        </View>

        {/* ── Stats Grid (UNCHANGED) ─────────────────────────────────────── */}
        <View style={styles.statsGrid}>
          <View style={styles.statCard}>
            <Text style={[styles.statValue, { color: selectedTheme.primary }]}>{stats.totalHabits}</Text>
            <Text style={styles.statLabel}>Active</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={[styles.statValue, { color: selectedTheme.primary }]}>{stats.completedHabits}</Text>
            <Text style={styles.statLabel}>Done</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={[styles.statValue, { color: selectedTheme.primary }]}>{stats.bestStreak}</Text>
            <Text style={styles.statLabel}>Best</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={[styles.statValue, { color: selectedTheme.primary }]}>{stats.totalStreaks}</Text>
            <Text style={styles.statLabel}>Total</Text>
          </View>
        </View>

        {/* ── Theme Selector (UNCHANGED) ─────────────────────────────────── */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>🎨 Change Theme</Text>
          <View style={styles.themeRow}>
            {themes.map(theme => (
              <TouchableOpacity
                key={theme.name}
                style={[
                  styles.themeBtn,
                  {
                    backgroundColor: theme.bg,
                    borderWidth: selectedTheme.name === theme.name ? 2 : 1,
                    borderColor:
                      selectedTheme.name === theme.name ? theme.primary : '#E0E0E0',
                  },
                ]}
                activeOpacity={0.8}
                onPress={() => setSelectedTheme(theme)}
              >
                <View style={[styles.themeDot, { backgroundColor: theme.primary }]} />
                <Text style={styles.themeName}>{theme.name}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>

        {/* ── Achievements (UNCHANGED) ───────────────────────────────────── */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>🏆 Achievements</Text>
          <View style={styles.achievementsGrid}>
            {achievements.map(a => (
              <View key={a.id} style={styles.achievementCard}>
                <Text style={styles.achievementIcon}>{a.icon}</Text>
                <Text style={styles.achievementName}>{a.name}</Text>
                <Text style={styles.achievementDesc}>{a.desc}</Text>
              </View>
            ))}
          </View>
        </View>

        {/* ── NEW: Weekly Activity Heatmap ──────────────────────────────── */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>📅 This Week's Activity</Text>
          <View style={newStyles.weekRow}>
            {WEEK_DAYS.map((day, i) => (
              <View key={day} style={newStyles.dayCol}>
                <View
                  style={[
                    newStyles.dayDot,
                    weeklyActivity[i]
                      ? { backgroundColor: selectedTheme.primary }
                      : { backgroundColor: '#E0E0E0' },
                  ]}
                />
                <Text style={newStyles.dayLabel}>{day}</Text>
              </View>
            ))}
          </View>
          <Text style={newStyles.weekCaption}>
            {weeklyActivity.filter(Boolean).length} / 7 days active this week
          </Text>
        </View>

        {/* ── NEW: Habit Completion Progress ────────────────────────────── */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>📊 Completion Overview</Text>
          <View style={newStyles.progressRow}>
            <Text style={[newStyles.progressPct, { color: selectedTheme.primary }]}>
              {completionPct}%
            </Text>
            <View style={{ flex: 1, marginLeft: 12 }}>
              <AnimatedBar percent={completionPct} color={selectedTheme.primary} />
              <Text style={newStyles.progressCaption}>
                {stats.completedHabits} of {stats.totalHabits} habits completed
              </Text>
            </View>
          </View>
        </View>

        {/* ── NEW: Top Habits Leaderboard ───────────────────────────────── */}
        {topHabits.length > 0 && (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>🥇 Top Habits by Streak</Text>
            {topHabits.map((h, idx) => (
              <View key={h.id} style={newStyles.leaderRow}>
                <Text style={newStyles.leaderRank}>#{idx + 1}</Text>
                <Text style={newStyles.leaderName} numberOfLines={1}>
                  {h.name}
                </Text>
                <View style={{ flex: 1, marginHorizontal: 10 }}>
                  <AnimatedBar
                    percent={Math.min(100, Math.round((h.streak / (h.duration || 1)) * 100))}
                    color={selectedTheme.primary}
                  />
                </View>
                <Text style={[newStyles.leaderStreak, { color: selectedTheme.primary }]}>
                  {h.streak}🔥
                </Text>
              </View>
            ))}
          </View>
        )}

        {/* ── NEW: Notification Settings ────────────────────────────────── */}
        <TouchableOpacity
          style={[styles.section, newStyles.notifTrigger]}
          activeOpacity={0.85}
          onPress={() => setShowNotifModal(true)}
        >
          <Text style={styles.sectionTitle}>🔔 Notification Settings</Text>
          <Text style={newStyles.notifSubtitle}>
            {Object.values(notifSettings).filter(Boolean).length} of 2 enabled · Tap to manage
          </Text>
        </TouchableOpacity>

        {/* ── NEW: Quick Tips ───────────────────────────────────────────── */}
        <View style={[styles.section, { backgroundColor: selectedTheme.motivationBg }]}>
          <Text style={styles.sectionTitle}>💡 Quick Tips</Text>
          {[
            'Start with just 2 minutes to build a habit.',
            'Link new habits to existing routines.',
            'Track progress visually for motivation.',
          ].map((tip, i) => (
            <View key={i} style={newStyles.tipRow}>
              <Text style={[newStyles.tipBullet, { color: selectedTheme.primary }]}>•</Text>
              <Text style={newStyles.tipText}>{tip}</Text>
            </View>
          ))}
        </View>

      </ScrollView>

      {/* ── NEW: Notification Settings Modal ─────────────────────────────── */}
      <Modal
        visible={showNotifModal}
        transparent
        animationType="slide"
        onRequestClose={() => setShowNotifModal(false)}
      >
        <Pressable style={newStyles.modalOverlay} onPress={() => setShowNotifModal(false)}>
          <Pressable style={newStyles.modalSheet} onPress={e => e.stopPropagation()}>
            <Text style={newStyles.modalTitle}>🔔 Notifications</Text>

            {(
              [
                { key: 'dailyReminder', label: 'Daily Reminder', icon: '⏰' },
                { key: 'streakAlert', label: 'Streak Alert', icon: '🔥' },
                
              ] as { key: keyof NotifSettings; label: string; icon: string }[]
            ).map(item => (
              <View key={item.key} style={newStyles.notifRow}>
                <Text style={newStyles.notifIcon}>{item.icon}</Text>
                <Text style={newStyles.notifLabel}>{item.label}</Text>
                <Switch
                  value={notifSettings[item.key]}
                  onValueChange={v =>
                    setNotifSettings(prev => ({ ...prev, [item.key]: v }))
                  }
                  trackColor={{ false: '#E0E0E0', true: selectedTheme.primary }}
                  thumbColor="#fff"
                />
              </View>
            ))}

            <TouchableOpacity
              style={[newStyles.modalClose, { backgroundColor: selectedTheme.primary }]}
              onPress={() => setShowNotifModal(false)}
            >
              <Text style={newStyles.modalCloseText}>Done</Text>
            </TouchableOpacity>
          </Pressable>
        </Pressable>
      </Modal>
    </View>
  );
}

// ─── ORIGINAL STYLES (completely unchanged) ──────────────────────────────────
const styles = StyleSheet.create({
  root: { flex: 1 },
  container: { padding: 0, paddingBottom: 40 },
  header: { padding: 24, borderRadius: 10, alignItems: 'center', marginBottom: 7 },
  name: { color: COLORS.card, fontSize: 22, fontWeight: 'bold' },
  tagline: { color: COLORS.lightOrange, fontSize: 14, marginTop: 4 },

  motivationContainer: {
    margin: 10,
    paddingVertical: 10,
    paddingHorizontal: 16,
    borderRadius: 12,
    alignItems: 'center',
    justifyContent: 'center',
    marginBottom: 12,
    minHeight: 60,
    borderWidth: 1,
  },
  quoteText: {
    fontSize: 14,
    fontStyle: 'italic',
    textAlign: 'center',
    lineHeight: 18,
  },
  quoteAuthor: {
    fontSize: 12,
    fontWeight: '600',
    marginTop: 4,
    textAlign: 'center',
  },

  statsGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 24, margin: 5, },
  statCard: {
    flex: 1,
    minWidth: '22%',
    backgroundColor: COLORS.card,
    padding: 16,
    borderRadius: 12,
    alignItems: 'center',
  },
  statValue: { fontSize: 24, fontWeight: 'bold' },
  statLabel: { fontSize: 12, color: COLORS.textLight, marginTop: 2 },
  levelBadge: { marginTop: 12, backgroundColor: 'rgba(255,255,255,0.2)', paddingHorizontal: 14, paddingVertical: 6, borderRadius: 20 },
  levelText: { color: '#fff', fontSize: 12, fontWeight: '700' },

  section: { backgroundColor: COLORS.card, padding: 16, borderRadius: 16, marginBottom: 16, margin: 10, },
  sectionTitle: { fontSize: 16, fontWeight: '600', color: COLORS.textDark, marginBottom: 12 },
  themeRow: { flexDirection: 'row', gap: 12 },
  themeBtn: { flex: 1, padding: 12, borderRadius: 12, alignItems: 'center' },
  themeDot: { width: 24, height: 24, borderRadius: 12, marginBottom: 6 },
  themeName: { fontSize: 12, color: COLORS.textDark, fontWeight: '500' },

  achievementsGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  achievementCard: {
    flex: 1,
    minWidth: '45%',
    backgroundColor: '#F5F5F5',
    padding: 14,
    borderRadius: 12,
    alignItems: 'center',
  },
  achievementIcon: { fontSize: 28, marginBottom: 6 },
  achievementName: { fontSize: 13, fontWeight: '600', color: COLORS.textDark },
  achievementDesc: { fontSize: 10, color: COLORS.textLight, textAlign: 'center', marginTop: 2 },
});

// ─── NEW STYLES (additions only) ─────────────────────────────────────────────
const newStyles = StyleSheet.create({
  // Milestone
  milestoneCard: { borderLeftWidth: 4, borderLeftColor: '#FF9800' },
  milestoneRow: { flexDirection: 'row', alignItems: 'flex-start' },
  milestoneIcon: { fontSize: 40 },
  milestoneLabel: { fontSize: 18, fontWeight: '700' },
  milestoneHint: { fontSize: 12, color: COLORS.textLight, marginTop: 2 },

  // Weekly heatmap
  weekRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 },
  dayCol: { alignItems: 'center', flex: 1 },
  dayDot: { width: 28, height: 28, borderRadius: 8, marginBottom: 4 },
  dayLabel: { fontSize: 10, color: COLORS.textLight },
  weekCaption: { fontSize: 12, color: COLORS.textLight, textAlign: 'center', marginTop: 4 },

  // Progress overview
  progressRow: { flexDirection: 'row', alignItems: 'center' },
  progressPct: { fontSize: 28, fontWeight: '800', minWidth: 60 },
  progressCaption: { fontSize: 11, color: COLORS.textLight, marginTop: 4 },

  // Leaderboard
  leaderRow: {
    flexDirection: 'row',
    alignItems: 'center',
    marginBottom: 10,
  },
  leaderRank: { fontSize: 13, fontWeight: '700', color: COLORS.textLight, minWidth: 28 },
  leaderName: { fontSize: 13, color: COLORS.textDark, maxWidth: 90 },
  leaderStreak: { fontSize: 13, fontWeight: '700', minWidth: 42, textAlign: 'right' },

  // Notification trigger
  notifTrigger: { paddingBottom: 12 },
  notifSubtitle: { fontSize: 12, color: COLORS.textLight, marginTop: -4 },

  // Tips
  tipRow: { flexDirection: 'row', marginBottom: 6 },
  tipBullet: { fontSize: 16, marginRight: 8, lineHeight: 20 },
  tipText: { fontSize: 13, color: COLORS.textDark, flex: 1, lineHeight: 20 },

  // Modal
  modalOverlay: {
    flex: 1,
    backgroundColor: 'rgba(0,0,0,0.35)',
    justifyContent: 'flex-end',
  },
  modalSheet: {
    backgroundColor: '#fff',
    borderTopLeftRadius: 24,
    borderTopRightRadius: 24,
    padding: 24,
    paddingBottom: 36,
  },
  modalTitle: {
    fontSize: 18,
    fontWeight: '700',
    color: COLORS.textDark,
    marginBottom: 20,
  },
  notifRow: {
    flexDirection: 'row',
    alignItems: 'center',
    marginBottom: 18,
  },
  notifIcon: { fontSize: 22, marginRight: 12 },
  notifLabel: { flex: 1, fontSize: 15, color: COLORS.textDark },
  modalClose: {
    marginTop: 8,
    padding: 14,
    borderRadius: 14,
    alignItems: 'center',
  },
  modalCloseText: { color: '#fff', fontWeight: '700', fontSize: 15 },
});

